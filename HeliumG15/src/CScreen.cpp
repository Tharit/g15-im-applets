...
bool CScreen::Draw(CLCDGfx *pGfx)
{
	...
	for(int i=0;i<4;i++)
		...
		{
			pGfx->DrawLine(0,GetHeight()-5,(GetWidth()/2)-20,GetHeight()-5);

			pGfx->DrawRect(GetWidth()/2-20,GetHeight()-7,40,8);
			
			pGfx->DrawLine(GetWidth()/2+20,GetHeight()-5,GetWidth(),GetHeight()-5);
			break;
		}
...
void CScreen::SetButtonBitmap(int iButton, int iBitmap)
{
	...
	else
	{
		...
		m_ahBitmaps[iButton] = (HBITMAP)LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(iBitmap),
...
void CScreen::UpdateButtons()
{
	for (int i = 0; i < 4; i++)
	{
		if(m_abShowButtons[i])
			m_aButtons[i].Show(m_bHideButtons);
	}
}
